...
def main():
    ...
    parser.add_argument(
        ...
        help="モデル＆結果を保存するディレクトリ（例: data/processed）"
    )
    parser.add_argument(
        "--out", required=True,
    ...
    for start in range(0, len(df) - window_size - step, step):
        ...
        feat_json = model_path.with_suffix(".json")
        ...
        print(f"[INFO] 特徴量リスト保存: {feat_json}")

    # --- 最終的に選ばれた特徴量だけでCSVを出力 ---
    if final_selected_features:
        df_selected = df[final_selected_features + [target_col, "time"]]
        df_selected.to_csv(out_selected_csv, index=False)
        print(f"[INFO] 選択後特徴量CSVを保存: {out_selected_csv}")
    else:
        print("[WARNING] 特徴量選択が一度も行われませんでした。CSV出力をスキップします。")

    # --- 結果まとめCSV保存 ---
    results_df = pd.DataFrame(results)
    results_csv = output_dir / "auto_feature_selection_results.csv"
    results_df.to_csv(results_csv, index=False)
    print(f"[INFO] 自動特徴量選択結果をCSV保存完了: {results_csv}")
